...
class ModelComparison(): 
    ...
    def _compare_loss_curves(self):
        ...
        for name, data, color in models:
            ...
            ax1.plot(train_loss, label=name, color=color, linewidth=2, 
                    marker='o', markersize=3, markevery=max(1, len(train_loss) // 20))
        ...
        for name, data, color in models:
            ...
            ax2.plot(val_loss, label=name, color=color, linewidth=2,
                    marker='s', markersize=3, markevery=max(1, len(val_loss) // 20))
    ...
    def _compare_mae_curves(self):
        ...
        for name, mae_arr, color in models:
            plt.plot(mae_arr, label=name, color=color, linewidth=2,
                    marker='o', markersize=3, markevery=max(1, len(mae_arr) // 20))
    ...
    def _compare_mape_curves(self):
        ...
        for name, mape_arr, color in models:
            plt.plot(mape_arr, label=name, color=color, linewidth=2,
                    marker='s', markersize=3, markevery=max(1, len(mape_arr) // 20))
    ...
    def _compare_rmse_curves(self):
        ...
        for name, rmse_arr, color in models:
            plt.plot(rmse_arr, label=name, color=color, linewidth=2,
                    marker='^', markersize=3, markevery=max(1, len(rmse_arr) // 20))
    ...
    def _compare_predictions(self, sample_range=None):
        ...
        # Plot actual values
        plt.plot(x_range, targets, label='Actual', color='black', 
                linewidth=2.5, alpha=0.7, marker='o', markersize=4,
        ...
        for name, predictions, color, marker in models:
            ...
            plt.plot(x_range, predictions, label=f'{name} Predicted', 
                    color=color, linewidth=2, alpha=0.8, marker=marker,
                    markersize=3, markevery=max(1, len(predictions) // 30))
        
        plt.title('Predictions Comparison: All Models vs Actual', 
